...
const db=admin.firestore();exports.handler=async(e,t)=>{try{const[r,a]=await Promise.all([db.collection("customers").get(),db.collection("dailyUpdates").get()]),s=r.docs.map(e=>({id:e.id,...e.data()})),d=a.docs.map(e=>({id:e.id,...e.data()}));return{statusCode:200,body:JSON.stringify({customers:s,dailyUpdates:d})}}catch(e){return console.error(e),{statusCode:500,body:JSON.stringify({error:"Failed to fetch data"})}}};
